...
static const char *DAY_NAMES[CometBlue_LastDay] = {
...
void CometBlueClimate::update_id() {
...
void CometBlueClimate::update_schedule() {
  ESP_LOGI(TAG, "Requesting Schedule of %10llx...", address);

  bool success = with_connection([this]() {
    bool success = false;
    for (int day = CometBlue_FirstDay; day < CometBlue_LastDay; ++day) {
      if (!last_schedule[day].has_value()) {
        success = query_schedule((CometBlueDay)day) || success;
      }
    }
    return success;
  });
  ...
  bool success = with_connection([this, call]() {
    ...
    if (call.get_mode().has_value()) {
      ...
      case CLIMATE_MODE_OFF:
        calls += set_temperature(CometBlueBT_OFF_TEMP);
        break;
      }
    }
...
void CometBlueClimate::parse_state(const std::string &data) {
  ...
  if (state->mode.boost_mode) {
    target_temperature = CometBlueBT_ON_TEMP;
    mode = CLIMATE_MODE_HEAT;
  } else if (target_temperature == CometBlueBT_OFF_TEMP) {
...
void CometBlueClimate::parse_schedule(const std::string &data) {
  ...
  if (schedule->day > CometBlue_LastDay) {
...
void CometBlueClimate::parse_id(const std::string &data) {
